do $$declare a bigint:=934000001;b bigint:=934000002;c bigint:=934000003;g uuid;req uuid;result jsonb;again jsonb;beforea numeric;beforeb numeric;i integer;j integer;choices text[]:=array['rock','paper','scissors'];begin
 ...
 begin perform z_play(gen_random_uuid(),b,'duel_test_b','plinko',11,null,'[0,0,0,0,0,0,0,0]');raise exception 'Plinko exceeded max';exception when others then if sqlerrm<>'Invalid wager' then raise;end if;end;
 begin perform z_run_start(gen_random_uuid(),b,'duel_test_b','higher',11,7,null);raise exception 'Higher exceeded max';exception when others then if sqlerrm<>'Invalid wager' then raise;end if;end;
 begin perform z_run_start(gen_random_uuid(),b,'duel_test_b','mines',11,null,array[0]);raise exception 'Mines exceeded max';exception when others then if sqlerrm<>'Invalid wager' then raise;end if;end;

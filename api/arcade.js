...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');try{validateQuery(req);
 if(req.method==='GET'){await limit(req,'public-arcade',180);
  if(req.query?.activity==='1'){const before=req.query.before;if(before&&!/^\d{1,19}$/.test(before))return res.status(400).json({error:'Invalid page.'});return res.json({feed:await db('z_activity?or=(kind.neq.rank,username.not.ilike.ZeekFusion)&select=id,username,kind,title,amount,created_at&order=id.desc&limit=50'+(before?'&id=lt.'+before:''))});}
  ...
  if(req.query?.balanceLeaders==='1')return res.json({leaders:await db(`z_users?kick_user_id=neq.20306616&username=not.ilike.ZeekFusion&select=username,zs_balance,lifetime_zs&order=${rankColumn}.desc,username.asc&limit=5`)});
  if(req.query?.leaderboardOffset!==undefined){const offset=Number(req.query.leaderboardOffset);if(!Number.isSafeInteger(offset)||offset<0||offset>1000000)return res.status(400).json({error:'Invalid page.'});return res.json({leaders:await db(`z_users?kick_user_id=neq.20306616&username=not.ilike.ZeekFusion&select=username,zs_balance,lifetime_zs&order=${rankColumn}.desc,username.asc&limit=100&offset=${offset}`)});}
  const u=await session(req);const [games,feed,leaders,personal,ownGames,active]=await Promise.all([
   ...
   db('z_activity?or=(kind.neq.rank,username.not.ilike.ZeekFusion)&select=id,username,kind,title,amount,created_at&order=id.desc&limit=50'),

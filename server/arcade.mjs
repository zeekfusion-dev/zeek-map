...
export function wager(value,kind='plinko'){const n=typeof value==='number'?value:NaN;const duel=['coin','dice','rps'].includes(kind);if(!Number.isSafeInteger(n)||n<1||!Number.isSafeInteger(n*200)||(!duel&&n>10000))throw Object.assign(new Error(duel?'Choose a positive whole-number wager.':'Choose a whole-number wager from 1 to 10,000 Zs.'),{status:400});return n;}

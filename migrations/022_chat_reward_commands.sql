...
create function z_process_command(p_event text,p_user bigint,p_name text,p_message text,p_created timestamptz,p_command text,p_argument text) returns boolean language plpgsql security definer set search_path=public,pg_temp as $$
declare receipt text;reward uuid;matches integer;purchase jsonb;reply text;request_id uuid;begin
 if p_event is null or length(p_event) not between 1 and 200 or p_message is null or length(p_message) not between 1 and 200 or p_user is null or p_user<=0 or p_name is null or length(p_name) not between 1 and 100 or p_created is null or abs(extract(epoch from now()-p_created))>300 or p_command is null or p_command not in('balance','buy') or p_argument is null or length(p_argument)>5000 then raise exception 'Invalid event';end if;

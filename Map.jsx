import TravelVotes from './components/TravelVotes';
...
export default function TravelMap() {
  ...
  return (
    <div className="map-page" style={{ width: "100%", height: "calc(100vh - 150px)", minHeight: isMobile ? "650px" : "420px", background: "#020617", position: "relative", overflow: "visible" }}>
      ...
      <nav className="map-view-controls" aria-label="Map region">{[['world','World'],['usa','USA']].map(([value,label])=><button key={value} aria-pressed={viewMode===value} onClick={()=>{setViewMode(value);setSelectedPlace(null);setHoveredPlace(null);}} style={buttonStyle(viewMode===value,value==='world'?'#3b82f6':'#22d3ee')}>{tr(label)}</button>)}</nav>
